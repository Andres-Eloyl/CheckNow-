...
export default function CartPage() {
  ...
  const activeCart = activeTab === 'mine' ? myOrders : orders;
  const currentTotal = activeTab === 'mine' ? myTotal : tableTotal;

  const handleConfirmOrders = async () => {
    setIsConfirming(true);
    try {
      const result = await confirmAllOrders();
      setConfirmMsg(`¡${result.items_sent} item(s) enviados a cocina! 🎉`);
      ...
      <main className="flex-1 p-5 pb-[180px] max-w-2xl mx-auto w-full">
        ...
        <AnimatePresence>
          {confirmMsg && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="mb-4 p-4 bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 rounded-2xl font-bold text-center text-[14px]"
            >
              {confirmMsg}
            </motion.div>
          )}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-[50vh] text-center px-4">
            ...
            <button onClick={loadOrders} className="bg-primary text-white font-bold py-3 px-6 rounded-full">
              Reintentar
            </button>
          </div>
        ) : (
              ...
              <motion.div layout className="min-h-[50vh]">
                {activeCart.map((item) => (
                  <CartItemCard 
                    key={item.id} 
                    item={item} 
                    readOnly={activeTab === 'table' && item.session_user_id !== activeUserId} 
                  />
                ))}
              </motion.div>
            ) : (
          ...
          >
            <div className="bg-white/90 dark:bg-[#151516]/90 backdrop-blur-xl border border-slate-200/60 dark:border-white/10 rounded-[28px] p-5 shadow-[0_-15px_40px_-15px_rgba(0,0,0,0.1)] mb-4">
              <div className="flex justify-between items-end mb-5">
                <div>
                  <p className="text-[13px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Subtotal</p>
                </div>
              ...
              <div className="flex gap-3">
                {/* Confirm orders button (only if pending items exist) */}
                {pendingOrders.length > 0 && (
                  <button
                    onClick={handleConfirmOrders}
                    disabled={isConfirming}
                    className="flex-1 h-14 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[15px] rounded-[16px] flex justify-center items-center gap-2 transition-all active:scale-[0.98] shadow-md disabled:opacity-60"
                  >
                    <span className="material-symbols-outlined text-[20px]">send</span>
                    {isConfirming ? 'Enviando...' : `Enviar (${pendingOrders.length})`}
                  </button>
                )}
                
                <Link 
                  href="/split" 
                  className="flex-1 h-14 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold text-[15px] rounded-[16px] flex justify-center items-center gap-2 transition-all hover:bg-slate-800 dark:hover:bg-slate-100 active:scale-[0.98] shadow-md outline-none focus-visible:ring-4 focus-visible:ring-primary/40"
                >
                  Dividir y Pagar
