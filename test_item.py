...
try:
  ...
  cat_data = json.loads(response_cat.read().decode())
  print('CREATE CAT OK:', cat_data)

  url_item = f'http://localhost:8001/api/{slug}/menu/items'
  data_item = json.dumps({'name': 'Margarita', 'price_usd': 10.0, 'category_id': cat_data['id']}).encode('utf-8')
  req_item = urllib.request.Request(url_item, data_item, {'Content-Type': 'application/json', 'Authorization': f'Bearer {token}'}, method='POST')
  response_item = urllib.request.urlopen(req_item)
  print('CREATE ITEM OK:', response_item.read().decode())
